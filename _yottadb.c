...
static int validate_sequence_of_bytes(PyObject *sequence, int max_sequence_len,
                                      int max_bytes_len, char *error_message) {
  int ret = YDBPY_VALID;
  ...
  if (max_sequence_len < len_seq) {
    snprintf(error_message, YDBPY_MAX_REASON, YDBPY_ERRMSG_SEQUENCE_TOO_LONG,
             len_seq, max_sequence_len);
  ...
  for (i = 0; i < len_seq; i++) { /* check each item for a bytes object */
    ...
    if (!PyBytes_Check(item)) {
      snprintf(error_message, YDBPY_MAX_REASON,
               YDBPY_ERRMSG_ITEM_IN_SEQUENCE_NOT_BYTES, i);
    ...
    if (max_bytes_len < len_bytes) {
      snprintf(error_message, YDBPY_MAX_REASON, YDBPY_ERRMSG_BYTES_TOO_LONG,
               len_bytes, max_bytes_len);
 ...
 * containing Strings.
 */
bool convert_py_bytes_sequence_to_ydb_buffer_array(PyObject *sequence,
                                                   int sequence_len,
                                                   ydb_buffer_t *buffer_array) {
  bool done;
  ...
  seq = PySequence_Fast(sequence, "argument must be iterable");
  if (!seq) {
    PyErr_SetString(YDBPythonError,
                    "Can't convert none sequence to buffer array.");
    return false;
  }
  ...
  for (int i = 0; i < sequence_len; i++) {
    ...
    if (false == done) {
      ...
      Py_DECREF(seq);
      PyErr_SetString(YDBPythonError,
                      "failed to copy bytes object to buffer array");
      return false;
    }
  }
 ...
 */
PyObject *convert_ydb_buffer_array_to_py_tuple(ydb_buffer_t *buffer_array,
                                               int len) {
  int i;
  ...
  for (i = 0; i < len; i++)
    PyTuple_SetItem(return_tuple, i,
                    Py_BuildValue("y#", buffer_array[i].buf_addr,
                                  buffer_array[i].len_used));
...
static bool load_YDBKey(YDBKey *dest, PyObject *varname, PyObject *subsarray) {
  ...
  if (Py_None != subsarray) {
    ...
    dest->subs_used =
        Py_SAFE_DOWNCAST(sequence_len_ssize, Py_ssize_t, unsigned int);
    subsarray_y = (ydb_buffer_t *)calloc(dest->subs_used, sizeof(ydb_buffer_t));
    convert_success = convert_py_bytes_sequence_to_ydb_buffer_array(
        subsarray, dest->subs_used, subsarray_y);
    if (convert_success) {
      ...
      PyErr_SetString(YDBPythonError,
                      "failed to covert sequence to buffer array");
      return false;
    }
  } else {
    dest->subs_used = 0;
 ...
 */
static int validate_py_keys_sequence_bytes(PyObject *keys_sequence,
                                           char *error_message) {
  int ret = YDBPY_VALID;
  ...
  if (YDBPY_VALID == ret) {
    for (i = 0; i < len_keys; i++) {
      ...
      if (!key_seq || !(PyTuple_Check(key) || PyList_Check(key))) {
        snprintf(error_message, YDBPY_MAX_REASON,
                 YDBPY_ERRMSG_KEY_IN_SEQUENCE_NOT_LIST_OR_TUPLE, i);
        ...
        if (Py_None != subsarray) {
          ...
          if (YDBPY_VALID != ret) {
            snprintf(error_message, YDBPY_MAX_REASON * 2,
                     YDBPY_ERRMSG_KEY_IN_SEQUENCE_SUBSARRAY_INVALID, i,
                     error_sub_reason);
          }
        }
      }
 ...
 */
static bool convert_key_sequence_to_YDBKey_array(PyObject *sequence,
                                                 YDBKey *ret_keys) {
  bool success = true;
...
static void raise_YDBError(int status, ydb_buffer_t *error_string_buffer,
  ...
  char *error_name, *error_message, *err_field1, *err_field2, *err_field3,
      *err_field4;
  ...
  snprintf(full_error_message, YDB_MAX_ERRORMSG, "%s (%d):%s", error_name,
...
static PyObject *data(PyObject *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "y#|OK", kwlist, &varname,
                                   &varname_len_ssize, &subsarray, &tp_token))
    return NULL;

  /* validate */
  VALIDATE_AND_CONVERT_BYTES_LEN(varname_len_ssize, varname_len, YDB_MAX_IDENT,
                                 YDBPY_INVALID_VARNAME_TOO_LONG,
  ...
  if (!return_NULL) {
    ...
    status = ydb_data_st(tp_token, &error_string_buffer, &varname_y, subs_used,
                         subsarray_y, &ret_value);
...
static PyObject *delete_wrapper(PyObject *self, PyObject *args,
                                PyObject *kwds) {
  bool return_NULL = false;
  ...
  /* parse and validate */
  static char *kwlist[] = {"varname", "subsarray", "delete_type", "tp_token",
                           NULL};
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "y#|OiK", kwlist, &varname,
                                   &varname_len_ssize, &subsarray, &deltype,
                                   &tp_token))
    return NULL;
  ...
  /* Setup for Call */
  SETUP_BUFFER(varname, varname_y, varname_len, "delete_wrapper()",
               return_NULL);
  SETUP_SUBS(subsarray, subs_used, subsarray_y, return_NULL);
  YDB_MALLOC_BUFFER(&error_string_buffer, YDB_MAX_ERRORMSG);
  ...
  if (!return_NULL) {
    /* Call the wrapped function */
    status = ydb_delete_st(tp_token, &error_string_buffer, &varname_y,
                           subs_used, subsarray_y, deltype);
...
static PyObject *delete_excel(PyObject *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "|OK", kwlist, &varnames,
                                   &tp_token))
    return NULL;
  ...
  if (0 < namecount) {
    success = convert_py_bytes_sequence_to_ydb_buffer_array(varnames, namecount,
                                                            varnames_ydb);
    if (!success)
      return NULL;
  }

  status = ydb_delete_excl_st(tp_token, &error_string_buffer, namecount,
                              varnames_ydb);
...
static PyObject *get(PyObject *self, PyObject *args, PyObject *kwds) {
  ...
  if (!return_NULL) {
    /* Call the wrapped function */
    status = ydb_get_st(tp_token, &error_string_buffer, &varname_y, subs_used,
                        subsarray_y, &ret_value);
    ...
    /* Create Python object to return */
    if (!return_NULL)
      return_python_string = Py_BuildValue("y#", ret_value.buf_addr,
                                           (Py_ssize_t)ret_value.len_used);
  }
...
static PyObject *incr(PyObject *self, PyObject *args, PyObject *kwds) {
  ...
  /* parse and validate */
  static char *kwlist[] = {"varname", "subsarray", "increment", "tp_token",
                           NULL};
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "y#|Oy#K", kwlist, &varname,
                                   &varname_len_ssize, &subsarray, &increment,
  ...
  VALIDATE_AND_CONVERT_BYTES_LEN(increment_len_ssize, increment_len,
                                 YDB_MAX_STR, YDBPY_INVALID_BYTES_TOO_LONG,
                                 YDBPY_ERRMSG_BYTES_TOO_LONG);

  /* Setup for Call */
  SETUP_BUFFER(varname, varname_y, varname_len, "incr() for varname",
               return_NULL);
  SETUP_SUBS(subsarray, subs_used, subsarray_y, return_NULL);
  SETUP_BUFFER(increment, increment_y, increment_len, "incr() for increment",
               return_NULL);
  ...
  if (!return_NULL) {
    /* Call the wrapped function */
    status = ydb_incr_st(tp_token, &error_string_buffer, &varname_y, subs_used,
                         subsarray_y, &increment_y, &ret_value);
...
static PyObject *lock(PyObject *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "|OKK", kwlist, &keys,
                                   &timeout_nsec, &tp_token))
    return NULL;

  if (Py_None != keys) {
    validation_status =
        validate_py_keys_sequence_bytes(keys, validation_error_reason);
    if (YDBPY_VALID != validation_status) {
      snprintf(validation_error_message, YDBPY_MAX_ERRORMSG,
               YDBPY_ERRMSG_KEYS_INVALID, validation_error_reason);
  ...
  if (!return_NULL) {
    ...
    if (ffi_prep_cif(&call_interface, FFI_DEFAULT_ABI, number_of_arguments,
    ...
    if (YDB_LOCK_TIMEOUT == status) {
      PyErr_SetString(
          YDBTimeoutError,
          "Not able to acquire all requested locks in the specified time.");
      return_NULL = true;
...
static PyObject *lock_decr(PyObject *self, PyObject *args, PyObject *kwds) {
  ...
  if (!return_NULL) {
    /* Call the wrapped function */
    status = ydb_lock_decr_st(tp_token, &error_string_buffer, &varname_y,
                              subs_used, subsarray_y);
...
static PyObject *lock_incr(PyObject *self, PyObject *args, PyObject *kwds) {
  ...
  /* parse and validate */
  static char *kwlist[] = {"varname", "subsarray", "timeout_nsec", "tp_token",
                           NULL};
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "y#|OLK", kwlist, &varname,
                                   &varname_len_ssize, &subsarray,
                                   &timeout_nsec, &tp_token))
    return NULL;
  ...
  if (!return_NULL) {
    /* Call the wrapped function */
    status = ydb_lock_incr_st(tp_token, &error_string_buffer, timeout_nsec,
                              &varname_y, subs_used, subsarray_y);
...
static PyObject *node_next(PyObject *self, PyObject *args, PyObject *kwds) {
  bool return_NULL = false;
  int max_subscript_string, default_ret_subs_used, real_ret_subs_used,
      ret_subs_used, status, subs_used;
  Py_ssize_t varname_len_ssize;
  unsigned int varname_len;
  char *varname;
  uint64_t tp_token;
  ...
  if (!return_NULL) {
    /* Call the wrapped function */
    status =
        ydb_node_next_st(tp_token, &error_string_buffer, &varname_y, subs_used,
                         subsarray_y, &ret_subs_used, ret_subsarray);
    ...
    if (YDB_ERR_INSUFFSUBS == status) {
      ...
      ret_subsarray =
          empty_buffer_array(real_ret_subs_used, max_subscript_string);
      /* recall the wrapped function */
      status = ydb_node_next_st(tp_token, &error_string_buffer, &varname_y,
                                subs_used, subsarray_y, &ret_subs_used,
                                ret_subsarray);
    }
    ...
    if (!return_NULL)
      return_tuple =
          convert_ydb_buffer_array_to_py_tuple(ret_subsarray, ret_subs_used);
  }
  /* free allocated memory */
  YDB_FREE_BUFFER(&varname_y);
  FREE_BUFFER_ARRAY(subsarray_y, subs_used);
...
static PyObject *node_previous(PyObject *self, PyObject *args, PyObject *kwds) {
  ...
  if (!return_NULL) {
    /* Call the wrapped function */
    status = ydb_node_previous_st(tp_token, &error_string_buffer, &varname_y,
                                  subs_used, subsarray_y, &ret_subs_used,
                                  ret_subsarray);
...
static PyObject *set(PyObject *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "y#|Oy#K", kwlist, &varname,
                                   &varname_len_ssize, &subsarray, &value,
  ...
  VALIDATE_AND_CONVERT_BYTES_LEN(varname_len_ssize, varname_len, YDB_MAX_IDENT,
                                 ...
                                 YDBPY_ERRMSG_VARNAME_TOO_LONG);
  VALIDATE_SUBSARRAY(subsarray);
  VALIDATE_AND_CONVERT_BYTES_LEN(value_len_ssize, value_len, YDB_MAX_STR,
                                 YDBPY_INVALID_BYTES_TOO_LONG,
                                 YDBPY_ERRMSG_BYTES_TOO_LONG);

  value_len = Py_SAFE_DOWNCAST(value_len_ssize, Py_ssize_t, unsigned int);

  /* Setup for Call */
  SETUP_BUFFER(varname, varname_y, varname_len, "set() for varname",
               return_NULL);
  SETUP_SUBS(subsarray, subs_used, subsarray_y, return_NULL);
  YDB_MALLOC_BUFFER(&error_string_buffer, YDB_MAX_ERRORMSG);
  ...
  if (!return_NULL) {
    /* Call the wrapped function */
    status = ydb_set_st(tp_token, &error_string_buffer, &varname_y, subs_used,
                        subsarray_y, &value_buffer);
...
static PyObject *str2zwr(PyObject *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "y#|K", kwlist, &str,
                                   &str_len_ssize, &tp_token))
    return NULL;
  ...
  if (!return_NULL) {
    ...
    if (!return_NULL)
      return_value =
          Py_BuildValue("y#", zwr_buf.buf_addr, (Py_ssize_t)zwr_buf.len_used);
  }
  /* free allocated memory */
...
static PyObject *subscript_next(PyObject *self, PyObject *args,
                                PyObject *kwds) {
  bool return_NULL = false;
  ...
  /* Setup for Call */
  SETUP_BUFFER(varname, varname_y, varname_len, "subscript_next()",
               return_NULL);
  SETUP_SUBS(subsarray, subs_used, subsarray_y, return_NULL);
  YDB_MALLOC_BUFFER(&error_string_buffer, YDB_MAX_ERRORMSG);
  ...
  if (!return_NULL) {
    /* Call the wrapped function */
    status = ydb_subscript_next_st(tp_token, &error_string_buffer, &varname_y,
                                   subs_used, subsarray_y, &ret_value);
...
static PyObject *subscript_previous(PyObject *self, PyObject *args,
                                    PyObject *kwds) {
  bool return_NULL = false;
  int status, return_length, subs_used;
  Py_ssize_t varname_len_ssize;
  ...
  /* Setup for Call */
  SETUP_BUFFER(varname, varname_y, varname_len, "subscript_previous()",
               return_NULL);
  SETUP_SUBS(subsarray, subs_used, subsarray_y, return_NULL);
  YDB_MALLOC_BUFFER(&error_string_buffer, YDB_MAX_ERRORMSG);
  YDB_MALLOC_BUFFER(&ret_value, YDBPY_DEFAULT_SUBSCRIPT_LEN);
  ...
  if (!return_NULL) {
    /* Call the wrapped function */
    status =
        ydb_subscript_previous_st(tp_token, &error_string_buffer, &varname_y,
                                  subs_used, subsarray_y, &ret_value);
 ...
 */
static int callback_wrapper(uint64_t tp_token, ydb_buffer_t *errstr,
                            void *function_with_arguments) {
  ...
  if (NULL == return_value) {
    ...
    PyErr_SetString(PyExc_TypeError,
                    "Callback function must return value of type int.");
    return YDB_ERR_TPCALLBACKINVRETVAL;
  }
...
static PyObject *tp(PyObject *self, PyObject *args, PyObject *kwds) {
  ...
  uint64_t tp_token;
  char *transid;
  PyObject *callback, *callback_args, *callback_kwargs, *varnames,
  ...
  /* parse and validate */
  static char *kwlist[] = {"callback", "args",     "kwargs", "transid",
                           "varnames", "tp_token", NULL};
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "O|OOsOK", kwlist, &callback,
                                   &callback_args, &callback_kwargs, &transid,
                                   &varnames, &tp_token))
    return_NULL = true;
  ...
  if (!return_NULL) {
    ...
    YDB_MALLOC_BUFFER(&error_string_buffer, YDB_MAX_ERRORMSG);
    function_with_arguments =
        Py_BuildValue("(OOO)", callback, callback_args, callback_kwargs);
    namecount = 0;
    if (Py_None != varnames)
      namecount = PySequence_Length(varnames);
    ...
    if (0 < namecount) {
      success = convert_py_bytes_sequence_to_ydb_buffer_array(
          varnames, namecount, varname_buffers);
      if (!success)
        return NULL;
    }
    ...
    /* Call the wrapped function */
    status =
        ydb_tp_st(tp_token, &error_string_buffer, callback_wrapper,
                  function_with_arguments, transid, namecount, varname_buffers);

    /* check status for Errors and Raise Exception */
    if (YDB_ERR_TPCALLBACKINVRETVAL == status) {
      return_NULL = true;
    } else if (YDB_TP_RESTART == status) {
      PyErr_SetString(YDBTPRestart,
                      "tp() callback function returned 'YDB_TP_RESTART'.");
      return_NULL = true;
    } else if (YDB_TP_ROLLBACK == status) {
      PyErr_SetString(YDBTPRollback,
                      "tp() callback function returned 'YDB_TP_ROLLBACK'.");
      return_NULL = true;
...
static PyObject *zwr2str(PyObject *self, PyObject *args, PyObject *kwds) {
  ...
  /* parse and validate */
  static char *kwlist[] = {"input", "tp_token", NULL};
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "y#|K", kwlist, &zwr,
                                   &zwr_len_ssize, &tp_token))
    return NULL;
  ...
  if (!return_NULL) {
    ...
    if (!return_NULL)
      return_value =
          Py_BuildValue("y#", str_buf.buf_addr, (Py_ssize_t)str_buf.len_used);
  }
  YDB_FREE_BUFFER(&zwr_buf);
  YDB_FREE_BUFFER(&error_string_buf);
  YDB_FREE_BUFFER(&str_buf);
...
static PyMethodDef methods[] = {
    ...
    {"delete", (PyCFunction)delete_wrapper, METH_VARARGS | METH_KEYWORDS,
     "deletes node value or tree data at node"},
    ...
    {"get", (PyCFunction)get, METH_VARARGS | METH_KEYWORDS,
     "returns the value of a node or raises exception"},
    {"incr", (PyCFunction)incr, METH_VARARGS | METH_KEYWORDS,
     "increments value by the value specified by 'increment'"},
    ...
    {"set", (PyCFunction)set, METH_VARARGS | METH_KEYWORDS,
     "sets the value of a node or raises exception"},
    ...
    {"subscript_next", (PyCFunction)subscript_next,
     METH_VARARGS | METH_KEYWORDS,
     "returns the name of the next subscript at "
     "the same level as the one given"},
    {"subscript_previous", (PyCFunction)subscript_previous,
     METH_VARARGS | METH_KEYWORDS,
...
static struct PyModuleDef _yottadbmodule = {
    PyModuleDef_HEAD_INIT, "_yottadb", /* name of module */
...
PyMODINIT_FUNC PyInit__yottadb(void) {
  ...
  PyDict_SetItemString(module_dictionary, "YDB_DEL_TREE",
                       Py_BuildValue("i", YDB_DEL_TREE));
  PyDict_SetItemString(module_dictionary, "YDB_DEL_NODE",
                       Py_BuildValue("i", YDB_DEL_NODE));

  PyDict_SetItemString(module_dictionary, "YDB_SEVERITY_WARNING",
                       Py_BuildValue("i", YDB_SEVERITY_WARNING));
  PyDict_SetItemString(module_dictionary, "YDB_SEVERITY_SUCCESS",
                       Py_BuildValue("i", YDB_SEVERITY_SUCCESS));
  PyDict_SetItemString(module_dictionary, "YDB_SEVERITY_ERROR",
                       Py_BuildValue("i", YDB_SEVERITY_ERROR));
  PyDict_SetItemString(module_dictionary, "YDB_SEVERITY_INFORMATIONAL",
                       Py_BuildValue("i", YDB_SEVERITY_INFORMATIONAL));
  PyDict_SetItemString(module_dictionary, "YDB_SEVERITY_FATAL",
                       Py_BuildValue("i", YDB_SEVERITY_FATAL));

  PyDict_SetItemString(module_dictionary, "YDB_DATA_UNDEF",
                       Py_BuildValue("i", YDB_DATA_UNDEF));
  PyDict_SetItemString(module_dictionary, "YDB_DATA_VALUE_NODESC",
                       Py_BuildValue("i", YDB_DATA_VALUE_NODESC));
  PyDict_SetItemString(module_dictionary, "YDB_DATA_NOVALUE_DESC",
                       Py_BuildValue("i", YDB_DATA_NOVALUE_DESC));
  PyDict_SetItemString(module_dictionary, "YDB_DATA_VALUE_DESC",
                       Py_BuildValue("i", YDB_DATA_VALUE_DESC));
  PyDict_SetItemString(module_dictionary, "YDB_DATA_ERROR",
                       Py_BuildValue("i", YDB_DATA_ERROR));

  PyDict_SetItemString(module_dictionary, "YDB_RELEASE",
                       Py_BuildValue("i", YDB_RELEASE));

  PyDict_SetItemString(module_dictionary, "YDB_MAX_IDENT",
                       Py_BuildValue("i", YDB_MAX_IDENT));
  PyDict_SetItemString(module_dictionary, "YDB_MAX_NAMES",
                       Py_BuildValue("i", YDB_MAX_NAMES));
  PyDict_SetItemString(module_dictionary, "YDB_MAX_STR",
                       Py_BuildValue("i", YDB_MAX_STR));
  PyDict_SetItemString(module_dictionary, "YDB_MAX_SUBS",
                       Py_BuildValue("i", YDB_MAX_SUBS));
  PyDict_SetItemString(module_dictionary, "YDB_MAX_TIME_NSEC",
                       Py_BuildValue("L", YDB_MAX_TIME_NSEC));
  PyDict_SetItemString(module_dictionary, "YDB_MAX_YDBERR",
                       Py_BuildValue("i", YDB_MAX_YDBERR));
  PyDict_SetItemString(module_dictionary, "YDB_MAX_ERRORMSG",
                       Py_BuildValue("i", YDB_MAX_ERRORMSG));

  PyDict_SetItemString(module_dictionary, "YDB_MIN_YDBERR",
                       Py_BuildValue("i", YDB_MIN_YDBERR));

  PyDict_SetItemString(module_dictionary, "YDB_OK", Py_BuildValue("i", YDB_OK));

  PyDict_SetItemString(module_dictionary, "YDB_INT_MAX",
                       Py_BuildValue("i", YDB_INT_MAX));
  PyDict_SetItemString(module_dictionary, "YDB_TP_RESTART",
                       Py_BuildValue("i", YDB_TP_RESTART));
  PyDict_SetItemString(module_dictionary, "YDB_TP_ROLLBACK",
                       Py_BuildValue("i", YDB_TP_ROLLBACK));
  PyDict_SetItemString(module_dictionary, "YDB_NOTOK",
                       Py_BuildValue("i", YDB_NOTOK));
  PyDict_SetItemString(module_dictionary, "YDB_LOCK_TIMEOUT",
                       Py_BuildValue("i", YDB_LOCK_TIMEOUT));

  PyDict_SetItemString(module_dictionary, "YDB_NOTTP",
                       Py_BuildValue("i", YDB_NOTTP));

  PyDict_SetItemString(module_dictionary, "YDB_ERR_TPTIMEOUT",
                       Py_BuildValue("i", YDB_ERR_TPTIMEOUT));
