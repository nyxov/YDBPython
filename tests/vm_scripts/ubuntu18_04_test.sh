# scp to freshly installed Ubuntu Server 18.04 VM with ssh installed
# run with bash
...
# install YottaDB
sudo apt install -y binutils pkg-config
...
# install YDBPython
sudo apt install -y python3-setuptools python3-dev libffi-dev
