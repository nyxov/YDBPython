...
#define YDBPY_ERRMSG_ITEM_IN_SEQUENCE_NOT_BYTES                                \
  "item %ld is not of type 'bytes'"

#define YDBPY_INVALID_KEY_IN_SEQUENCE_NOT_LIST_OR_TUPLE -103
#define YDBPY_ERRMSG_KEY_IN_SEQUENCE_NOT_LIST_OR_TUPLE                         \
  "item %ld is not a list or tuple."

#define YDBPY_INVALID_KEY_IN_SEQUENCE_VARNAME_NOT_BYTES -104
#define YDBPY_ERRMSG_KEY_IN_SEQUENCE_VARNAME_NOT_BYTES                         \
  "item %ld in key sequence invalid: first element must be of type 'bytes'"
...
#define YDBPY_ERRMSG_KEY_IN_SEQUENCE_INCORECT_LENGTH                           \
  "item %ld must be length 1 or 2."

#define YDBPY_INVALID_KEY_IN_SEQUENCE_VARNAME_TOO_LONG -205
#define YDBPY_ERRMSG_KEY_IN_SEQUENCE_VARNAME_TOO_LONG                          \
  "item %ld in key sequence has invalid varname length %ld: max %d."

#define YDBPY_ERRMSG_KEY_IN_SEQUENCE_SUBSARRAY_INVALID                         \
  "item %ld in key sequence has invalid subsarray: %s"
...
#define SETUP_BUFFER(PYVARNAME, YDBVARNAME, VARNAMELEN, FUNCTIONNAME,          \
                     RETURN_NULL)                                              \
  {                                                                            \
    bool copy_success;                                                         \
    YDB_MALLOC_BUFFER(&(YDBVARNAME), (VARNAMELEN));                            \
    YDB_COPY_BYTES_TO_BUFFER((PYVARNAME), (VARNAMELEN), &(YDBVARNAME),         \
                             copy_success);                                    \
    if (!copy_success) {                                                       \
      PyErr_Format(YDBPythonError, "YDB_COPY_BYTES_TO_BUFFER failed in %s",    \
                   (FUNCTIONNAME));                                            \
      (RETURN_NULL) = true;                                                    \
    }                                                                          \
  }
  ...
  {                                                                            \
    ...
    if (Py_None != SUBSARRAY_PY) {                                             \
      ...
      success = convert_py_bytes_sequence_to_ydb_buffer_array(                 \
          SUBSARRAY_PY, SUBSUSED, SUBSARRAY_YDB);                              \
      if (!success)                                                            \
        RETURN_NULL = true;                                                    \
    }                                                                          \
  }
...
#define VALIDATE_AND_CONVERT_BYTES_LEN(ORIGINAL_LEN, CONVERTED_LEN, MAX_LEN,   \
                                       LEN_ERR, LEN_ERR_MSG)                   \
  {                                                                            \
    if ((MAX_LEN) < (ORIGINAL_LEN)) {                                          \
      char validation_error_message[YDBPY_MAX_ERRORMSG];                       \
      snprintf(validation_error_message, YDBPY_MAX_ERRORMSG, LEN_ERR_MSG,      \
               ORIGINAL_LEN, MAX_LEN);                                         \
      raise_ValidationError(LEN_ERR, validation_error_message);                \
      return NULL;                                                             \
    } else {                                                                   \
      CONVERTED_LEN =                                                          \
          Py_SAFE_DOWNCAST(ORIGINAL_LEN, Py_ssize_t, unsigned int);            \
    }                                                                          \
  }

#define VALIDATE_SEQUENCE_OF_BYTES_INPUT(SEQUENCE, MAX_SEQUENCE_LEN,           \
                                         MAX_BYTES_LEN, OUTER_ERROR_MESSAGE)   \
  {                                                                            \
    if (Py_None != SEQUENCE) { /* allow None */                                \
      char validation_reason_message[YDBPY_MAX_REASON];                        \
      int validation_status = validate_sequence_of_bytes(                      \
          SEQUENCE, MAX_SEQUENCE_LEN, MAX_BYTES_LEN,                           \
          validation_reason_message);                                          \
      if (YDBPY_VALID != validation_status) {                                  \
        char validation_error_message[YDBPY_MAX_ERRORMSG];                     \
        snprintf(validation_error_message, YDBPY_MAX_ERRORMSG,                 \
                 OUTER_ERROR_MESSAGE, validation_reason_message);              \
...
#define VALIDATE_SUBSARRAY(SUBSARRAY)                                          \
  {                                                                            \
    VALIDATE_SEQUENCE_OF_BYTES_INPUT(SUBSARRAY, YDB_MAX_SUBS, YDB_MAX_STR,     \
                                     YDBPY_ERRMSG_SUBSARRAY_INVALID)           \
  }

#define VALIDATE_VARNAMES(VARNAMES)                                            \
  {                                                                            \
    VALIDATE_SEQUENCE_OF_BYTES_INPUT(VARNAMES, YDB_MAX_NAMES, YDB_MAX_IDENT,   \
                                     YDBPY_ERRMSG_VARNAME_INVALID)             \
  }
